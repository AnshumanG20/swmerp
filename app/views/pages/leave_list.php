...
<div id="content-container">
    <div id="page-head">
        ...
        <li class="active">Leave Type List</li>
        </ol>
        <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
        <!--End breadcrumb-->
            ...
            <div class="col-sm-12">
                <div class="panel">
                    <div class="panel-heading">
                        <h5 class="panel-title">Leave Type List</h5>
                    </div>
                    <div class="panel-body">
                        <div class="pad-btm">
                            <a href="<?=URLROOT;?>/Masters/leave_add_update"><button id="demo-foo-collapse" class="btn btn-purple">Add New  <i class="fa fa-arrow-right"></i></button></a>
                        </div>
...
<script type="text/javascript">
	$(document).ready(function() {
        ...
        <?php if ($msgg = flashToast("leaveTypeAddSuccess")) { ?>
            modelInfo("<?=$msgg;?>"); 
        <?php } ?>
        <?php if ($msgg = flashToast("leaveTypeAddError")) { ?>
            modelDanger("<?=$msgg;?>"); 
        <?php } ?>
        <?php if ($msgg = flashToast("leaveTypeUpdateSuccess")) { ?>
            modelInfo("<?=$msgg;?>"); 
        <?php } ?>
        <?php if ($msgg = flashToast("leaveDeletSuccess")) { ?>
            modelInfo("<?=$msgg;?>"); 
        <?php } ?>
 ...
 function deletefun(ID)
{
    ...
    if(result)
     window.location.replace("<?=URLROOT;?>/Masters/DeleteByIdLeave/"+ID);
}
</script>