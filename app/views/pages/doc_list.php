					    ...
					    <div class="col-sm-12">
					        <div class="panel">
                                ...
                                <div class="panel-body">
                                     <div class="pad-btm">
                                        <a href="<?=URLROOT;?>/Masters/doc_add_update"><button id="demo-foo-collapse" class="btn btn-purple">Add New  <i class="fa fa-arrow-right"></i></button></a>
                                    </div>
                                    ...
                                    <?php else:
                                            $i=0;
                                            foreach ($data["doclist"] as $value):
                                    ?>
                                            <tr>
                                                <td><?=++$i;?></td>
                                                <td><?=$value["doc_name"];?></td>
                                                <td>
                                                    <a class="btn btn-primary" href="<?=URLROOT;?>/Masters/doc_add_update/<?=$value["_id"];?>" role="button">Edit</a>
                                                        &nbsp;&nbsp;
...
<script type="text/javascript">
	$(document).ready(function() {
        ...
        <?php if ($msgg = flashToast("documentExist")) { ?>
            modelDanger("<?=$msgg;?>"); 
        <?php } ?>
        <?php if ($msgg = flashToast("documentAddSuccess")) { ?>
            modelInfo("<?=$msgg;?>"); 
        <?php } ?>

        <?php if ($msgg = flashToast("documentAddError")) { ?>
            modelDanger("<?=$msgg;?>"); 
        <?php } ?>
        <?php if ($msgg = flashToast("documentUpdateSuccess")) { ?>
            modelInfo("<?=$msgg;?>"); 
        <?php } ?>
 ...
 function deletefun(ID)
{
    ...
    if(result)
     window.location.replace("<?=URLROOT;?>/Masters/DeleteByIdDoc/"+ID);
}
</script>