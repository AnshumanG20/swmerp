...
<!--CONTENT CONTAINER-->
            ...
            <div id="content-container">
                <div id="page-head">
                    ...
                    <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
                    <div id="page-title">
                       <!-- <h1 class="page-header text-overflow">Course List</h1>//-->
                    </div>
                    <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
                    <!--End page title-->
					    ...
					    <div class="col-sm-12">
					        <div class="panel">
					            <div class="panel-heading">
					                <h5 class="panel-title">Qualification List</h5>

					            </div>
                                <div class="panel-body">
                                    <div class="pad-btm">
                                        <a href="<?=URLROOT;?>/Masters/qualification_add_update"><button id="demo-foo-collapse" class="btn btn-purple">Add New  <i class="fa fa-arrow-right"></i></button></a>
                                    </div>
					                <table id="demo_dt_basic" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                        ...
                                        <tbody>
                                    <?php
                                    if(isset($data["qualificationlist"])):
                                    ...
                                    <?php else:
                                            $i=0;
                                            foreach ($data["qualificationlist"] as $value):
                                    ?>
                                            <tr>
                                                <td><?=++$i;?></td>
                                                <td><?=$value["course_name"];?></td>
                                                <td>
                                                    <a class="btn btn-primary" href="<?=URLROOT;?>/Masters/qualification_add_update/<?=$value["_id"];?>" role="button">Edit</a>
                                                        &nbsp;&nbsp;
...
<script type="text/javascript">
	$(document).ready(function() {
        ...
        <?php if ($msgg = flashToast("QualAddSuccess")) { ?>
            modelInfo("<?=$msgg;?>"); 
        <?php } ?>

        <?php if ($msgg = flashToast("QualAddError")) { ?>
            modelDanger("<?=$msgg;?>"); 
        <?php } ?>
        <?php if ($msgg = flashToast("QualUpdateSuccess")) { ?>
            modelInfo("<?=$msgg;?>"); 
        <?php } ?>
        <?php if ($msgg = flashToast("QualDeleteSuccess")) { ?>
            modelInfo("<?=$msgg;?>"); 
        <?php } ?>
 ...
 function deletefun(ID)
{
    ...
    if(result)
     window.location.replace("<?=URLROOT;?>/Masters/DeleteByIdQualification/"+ID);
}
</script>