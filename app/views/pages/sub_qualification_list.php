...
<!--CONTENT CONTAINER-->
            ...
            <div id="content-container">
                <div id="page-head">
					...
					<li class="active">Stream List</li>
                    </ol>
                    <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
                    <!--End breadcrumb-->
					    ...
					    <div class="col-sm-12">
					        <div class="panel">
					            <div class="panel-heading">
					                <h5 class="panel-title">Stream List</h5>

					            </div>
                                <div class="panel-body">
                                     <div class="pad-btm">
                                        <a href="<?=URLROOT;?>/Masters/sub_qualification_add_update"><button id="demo-foo-collapse" class="btn btn-purple">Add New <i class="fa fa-arrow-right"></i></button></a>
                                    </div>
					                <table id="demo_dt_basic" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                        ...
                                        <tbody>
                                    <?php
                                    if(isset($data["sub_qualificationlist"])):
                                          if($data["sub_qualificationlist"]==""):
                                    ?>
                                            <tr>
                                    ...
                                    <?php else:
                                            $i=0;
                                            foreach ($data["sub_qualificationlist"] as $value):
                                    ?>
                                            <tr>
                                                <td><?=++$i;?></td>
                                                ...
                                                <td>
                                                    <a class="btn btn-primary" href="<?=URLROOT;?>/Masters/sub_qualification_add_update/<?=$value["_id"];?>" role="button">Edit</a>
                                                        &nbsp;&nbsp;
...
<script type="text/javascript">
	$(document).ready(function() {
        ...
        <?php if ($msgg = flashToast("subQualAddSuccess")) { ?>
            modelInfo("<?=$msgg;?>"); 
        <?php } ?>

        <?php if ($msgg = flashToast("subQualAddError")) { ?>
            modelDanger("<?=$msgg;?>"); 
        <?php } ?>
        <?php if ($msgg = flashToast("subQualUpdateSuccess")) { ?>
            modelInfo("<?=$msgg;?>"); 
        <?php } ?>
        <?php if ($msgg = flashToast("subQualDeleteSuccess")) { ?>
            modelInfo("<?=$msgg;?>"); 
        <?php } ?>
 ...
 function deletefun(ID)
{
    ...
    if(result)
     window.location.replace("<?=URLROOT;?>/Masters/sub_DeleteByIdQualification/"+ID);
}
</script>