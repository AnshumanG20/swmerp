...
<!--CONTENT CONTAINER-->
            ...
            <div id="content-container">
                <div id="page-head">
					...
					<li class="active">Employment Type List</li>
                    </ol>
                    <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
                    <!--End breadcrumb-->
					    ...
					    <div class="col-sm-12">
					        <div class="panel">
					            <div class="panel-heading">
					                <h5 class="panel-title">Employment Type List</h5>
					            </div>
                                <div class="panel-body">
                                    <div class="pad-btm">
                                        <a href="<?=URLROOT;?>/Masters/employeement_add_update"><button id="demo-foo-collapse" class="btn btn-purple">Add New  <i class="fa fa-arrow-right"></i></button></a>
                                    </div>
					                <table id="demo_dt_basic" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                        ...
                                        <tbody>
                                    <?php
                                    if(isset($data["employeementlist"])):
                                          if($data["employeementlist"]==""):
                                    ?>
                                            <tr>
                                    ...
                                    <?php else:
                                            $i=0;
                                            foreach ($data["employeementlist"] as $value):
                                    ?>
                                            <tr>
                                                <td><?=++$i;?></td>
                                                <td><?=$value["employment_type"];?></td>
                                                <td>
                                                    <a class="btn btn-primary" href="<?=URLROOT;?>/Masters/employeement_add_update/<?=$value["_id"];?>" role="button">Edit</a>
                                                        &nbsp;&nbsp;
...
<script type="text/javascript">
	$(document).ready(function() {
        ...
        <?php if ($msgg = flashToast("EmpAddSuccess")) { ?>
            modelInfo("<?=$msgg;?>"); 
        <?php } ?>
        <?php if ($msgg = flashToast("EmpAddError")) { ?>
            modelDanger("<?=$msgg;?>"); 
        <?php } ?>
        <?php if ($msgg = flashToast("EmpTypeUpdateSuccess")) { ?>
            modelInfo("<?=$msgg;?>"); 
        <?php } ?>
        <?php if ($msgg = flashToast("EmpAddError")) { ?>
            modelDanger("<?=$msgg;?>"); 
        <?php } ?>
        <?php if ($msgg = flashToast("EmpDeleteSuccess")) { ?>
            modelInfo("<?=$msgg;?>"); 
        <?php } ?>
 ...
 function deletefun(ID)
{
    ...
    if(result)
     window.location.replace("<?=URLROOT;?>/Masters/DeleteByIdEmployeement/"+ID);
}
</script>