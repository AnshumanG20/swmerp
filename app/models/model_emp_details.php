...
class model_emp_details
{
    ...
    public function getEmpDetailsForAttendance(){
        $result = $this->db->table("tbl_emp_details")
                             ->select("first_name,last_name,biometric_employee_code")
                             ->where("_status", "=", 1)
                             ->where("biometric_employee_code", ">", 1)
                             ->orderBy('biometric_employee_code')
                             ->get();
                //get for not null biometricemployee_code
    ...
    public function getEmpDetailsNameById($ID){
       $result = $this->db->table($this->tbl_name)
                            ->select("*")
                            ->where("_id", "=", $ID)
                            ->first();
	...
	public function getJoinEmpDetails(){
      $result = $this->db->table($this->tbl_name)
                            ...
                            ->leftjoin('tbl_user_mstr','tbl_user_mstr._id', '=', 'tbl_emp_details.user_mstr_id')
                            ->select("tbl_emp_details._id, 
									  ...
									  tbl_emp_details.grade_mstr_id, 
                                      tbl_user_mstr.user_name,
    ...
    public function search_emp($data)
    {
            try
            {
                $sql =" select tbl_emp_details._id, 
                          ...
                          left join tbl_user_mstr on(tbl_user_mstr._id = tbl_emp_details.user_mstr_id)
                          where tbl_emp_details._id!=1";
                ...
                $sql .= "ORDER BY tbl_emp_details.first_name ASC";
                $this->db->query($sql);
    ...
    public function empActivate($ID){
        $result = $this->db->table($this->tbl_name)
                            ->where("_id", "=", $ID)
                            ->update(['_status'=>1]);
        if($result){
    ...
    public function empDeactivate($ID){
        $result = $this->db->table($this->tbl_name)
                            ->where("_id", "=", $ID)
                            ->where("_status", "=", 1)
                            ->update(['_status'=>0]);
